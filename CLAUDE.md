...
使用 `grill-with-docs` 完成方案追问后，不要立刻进入实施阶段。

1. 先询问用户是否结束 grilling。
2. 如果用户确认结束，先使用 `to-prd` 形成 PRD，再使用 `to-issues` 拆分可实施问题。
3. PRD 和 issues 完成后，再询问用户是否开始实施。
4. 进入实施时，先充分分析并拆解任务；考虑使用 subagent，把简单、独立、低风险的工作分配给 `gpt-5.4-mini`。
5. 编码完成后进入测试阶段前，询问用户选择：
   - 正常编写并运行测试用例
   - 只进行编译校验
